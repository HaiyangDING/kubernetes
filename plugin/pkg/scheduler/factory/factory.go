...
func (f *ConfigFactory) responsibleForPod(pod *api.Pod) bool {
	if f.SchedulerName == api.DefaultSchedulerName {
		if pod.Annotations[SchedulerAnnotationKey] == f.SchedulerName || pod.Annotations[SchedulerAnnotationKey] == "" {
			return true
		}
		return false
	} else {
		if pod.Annotations[SchedulerAnnotationKey] == f.SchedulerName {
			return true
		}
		return false
	}
